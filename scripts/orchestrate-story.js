...
function buildOrchestratorPrompt(story, epic, blueprint, designContext = null) {
  let prompt = `# Orchestrator: Story → Blueprint Translation
...
`;
  ...
  if (designContext) {
    prompt += `
## Design System Context (for Stitch UI Generation)
When generating Component Specifications for the visual track, use the following design system tokens and guidelines to inform your output:
\`\`\`markdown
${designContext}
\`\`\`
`;
...
${(getStitchConfig().enabled && classification !== 'behavioral') ? '- [ ] Stitch design generation (visual track)\n' : ''}- [ ] Jules TDA implementation
...
async function v3() {
  ...
  const stitchConfig = getStitchConfig();
  if (stitchConfig.enabled && stitchConfig.designSystemContext && !isRemoval) {
    const designPath = path.resolve(getRepoRoot(), stitchConfig.designSystemContext);
    if (fs.existsSync(designPath)) {
      designContext = fs.readFileSync(designPath, 'utf-8');
      console.log(`   🎨 Loaded Design System context from ${stitchConfig.designSystemContext}`);
    } else {
      console.warn(`   ⚠️ Stitch is enabled but design system file not found: ${stitchConfig.designSystemContext}`);
    }
  }
  ...
  const prompt = isRemoval
    ...
    : buildOrchestratorPrompt(story, epic, blueprint, designContext);
