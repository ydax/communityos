...
async function v3() {
  ...
  if (attempt > MAX) {
    try { execSync(`gh pr comment ${prNumber} --repo ${REPO} --body "🚨 Max revisions exceeded."`, { encoding: 'utf-8', stdio: 'pipe' }); } catch (e) {}
  ...
  if (parsed.decision === 'APPROVE') {
    ...
    try { execSync(`gh pr comment ${prNumber} --repo ${REPO} --body "🚨 Escalated: ${parsed.reason}"`, { encoding: 'utf-8', stdio: 'pipe' }); } catch (e) {}
