...
export async function listListings(db, filters = {}) {
  try {
    ...
    // Order by creation date in memory to bypass Firebase composite index requirement
...
export async function createVariant(db, variantData) {
  ...
  try {
    ...
    const docRef = await variantsRef.add({
      ...
      ownerId: variantData.ownerId || null,
...
export async function listAllActiveListings(db, filters = {}) {
  try {
    ...
    // Order by creation date (most recent first) in memory instead of query
    // query = query.orderBy("createdAt", "desc");
