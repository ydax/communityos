...
async function generateVariations(input, { callGemini }) {
  ...
  // Available themes to ensure distinct variations
  const VALID_THEMES = ["the-trade", "the-maker", "the-venue"];
  const baseThemeIdx = VALID_THEMES.indexOf(baseConfig.theme);
  
  const currentVibeId = input.vibePreset;

  // Create variations by swapping theme + layout + vibe colors
  const variations = [baseConfig];

  for (let i = 0; i < 2 && variations.length < 3; i++) {
    // Pick a distinct alternative theme
    const altTheme = VALID_THEMES[(Math.max(0, baseThemeIdx) + i + 1) % VALID_THEMES.length];
    
    // Get valid layouts for this new theme
    const validLayouts = VALID_HERO_LAYOUTS[altTheme] || ["clean"];
    const altLayout = validLayouts[i % validLayouts.length];

    // Pick a vibe specific to this alternative theme
    const themeVibes = getVibesByTheme(altTheme);
    const altVibe = themeVibes.filter(v => v.id !== currentVibeId)[0] || themeVibes[0];

    const variant = JSON.parse(JSON.stringify(baseConfig)); // Deep clone

    // Swap theme
    variant.theme = altTheme;
